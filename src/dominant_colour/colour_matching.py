...
def lookup_indices(pixels) -> NDArray:
    ...
    return (
        red // BIN_WIDTH
        + BINS_PER_CHANNEL * (green // BIN_WIDTH)
        + BINS_PER_CHANNEL**2 * (blue // BIN_WIDTH)
    )
...
def rgb_to_colour_names(pixels) -> list[str]:
    ...
    return [LOOKUP_TABLE_COLUMNS[column] for column in most_likely]
